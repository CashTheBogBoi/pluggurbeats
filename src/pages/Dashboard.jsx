...
export default function Dashboard() {
  ...
  // ---- live profile + campaigns (React Query cache fed by onSnapshot) ----
  ...
  const profile = (profileDoc && !profileDoc.__error)
    ? { ...profileDoc, email: user?.email, avatarUrl: avatarUrl || undefined }
    : {};
  ...
  // Accounts kept after the data wipe have no user doc yet — create the default
  // one (allowed by firestore.rules create) so credits/campaigns work.
  useEffect(() => {
  ...
  // Resolve the avatar URL whenever avatarPath changes.
  useEffect(() => {
    const p = profileDoc?.avatarPath;
    if (p && p !== lastAvatarPath.current) {
      lastAvatarPath.current = p;
      getDownloadURL(ref(storage, p)).then(setAvatarUrl).catch(() => {});
    }
  }, [profileDoc?.avatarPath]);

  // One-time entitlement recovery for a missed monthly grant.
  useEffect(() => {
    ...
    if (grantMissing) {
      ...
      call("reconcileCredits").then((d) => { if (d?.granted) showToast(`Your ${cap(t)} credits are now active.`); }).catch(() => {});
    }
  }, [profileDoc]);
  ...
  async function startSubscription(plan, btn) {
    ...
    try {
      const data = await call("createSubscriptionCheckout", { plan });
  ...
  if (!authReady) return <div id="dash-root"><div style={{ padding: "120px 20px", textAlign: "center", color: "var(--bone-dim)" }}>Loading…</div></div>;
...
function Analytics({ campaigns, uid }) {
  ...
  // Aggregates come straight off the live campaign-doc counters.
  const sent = campaigns.reduce((s, c) => s + (Array.isArray(c.pitchedTo) ? c.pitchedTo.length : 0), 0);
  const opens = campaigns.reduce((s, c) => s + (c.opens || 0), 0);
  ...
  return (
    <section>
      ...
      <div className="card">
        ...
        {pitched.length
          ? pitched.map((c) => <CampaignActivity key={c.id} campaign={c} uid={uid} />)
          : <p className="hint">No engagement yet — opens and downloads appear here once recipients interact.</p>}
      </div>
    </section>
  );
}

// Subscribes live to one campaign's events subcollection and renders the
// per-contact open/download table. Mounted once per pitched campaign.
function CampaignActivity({ campaign, uid }) {
  const { data } = useLiveCollection(
    ["events", uid, campaign.id],
    () => collection(db, "users", uid, "campaigns", campaign.id, "events"),
    { enabled: !!uid }
  );
  const events = data || [];
  ...
  return (
    <div style={{ marginBottom: "18px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "8px", flexWrap: "wrap", gap: "8px" }}>
        <strong style={{ fontSize: "14px" }}>{title}</strong>
        <span className="mono hint" style={{ fontSize: "11px" }}>{campaign.opens || 0} opens · {campaign.downloads || 0} downloads</span>
      </div>
      <div style={{ overflowX: "auto" }}>
        <table>
          <thead><tr><th>Recipient</th><th>Opened</th><th>Downloaded</th><th>Last activity</th></tr></thead>
          <tbody>
            {rows.length ? rows.map(([email, r], i) => (
              <tr key={i}>
                <td className="mono" style={{ fontSize: "12px" }}>{email}</td>
                ...
                <td style={{ color: "var(--bone-dim)" }}>{fmtTime(r.last)}</td>
              </tr>
            )) : <tr><td colSpan="4" style={{ color: "var(--bone-dim)", textAlign: "center", padding: "18px" }}>Awaiting recipient activity…</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
function LoopDrops({ user, loopBalance, showToast }) {
  ...
  // Live list of this maker's loops (React Query cache fed by onSnapshot).
  const { data: myLoops } = useLiveCollection(
    ["myLoops", user?.uid],
    () => query(collection(db, "loops"), where("makerUid", "==", user.uid), orderBy("createdAt", "desc")),
    { enabled: !!user?.uid }
  );
  ...
  return (
    <section>
      ...
      <div className="card" style={{ marginTop: "18px" }}>
        ...
        {!myLoops ? <p className="hint" style={{ textAlign: "center", padding: "20px" }}>Loading…</p>
...
function ProfileModal({ user, profile, onClose }) {
  ...
  async function save() {
    ...
    try {
      ...
      // The live profile listener picks up the new fields (and avatar) automatically.
